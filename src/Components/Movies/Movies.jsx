...
export default function Movies() {
  ...
  function getTvSeries(MovieItems, func) {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/${MovieItems}?api_key=ace717a114ddbadf2b9c19aa6022f8ac`
      )
      .then((response) => {
  ...
  useEffect(() => {
    getTvSeries("now_playing", setTvSeries);
    getTvSeries("upcoming", setOnTheAir);
...

        {
          maxVotedMovie.vote_average  ? (
            <div className=" ps-5  d-flex justify-content-center align-items-start flex-column flex-grow-0 ">
              ...
              <h1 className="">  {maxVotedMovie.title }</h1>
              <h2 className="h4">
